...
public class PrintTest1 implements Printable {
	...
	public int print(Graphics gra, PageFormat pf, int pageIndex)
		 ...
		 * System.out.println(paper.getHeight()); pf.setPaper(paper);//将该纸张作为格式
		 */
		...
		case 0:
			...
			// -2- 直接构造TextLayout打印
			/*
			 * FontRenderContext frc = g2.getFontRenderContext(); TextLayout
			 * layout = new TextLayout(str, font, frc); layout.draw(g2,
			 * (float)x, (float)y+2*heigth);
			 */
			// -3- 用LineBreakMeasurer进行打印
			/*
			 * AttributedString text = new AttributedString(str);
			 * text.addAttribute(TextAttribute.FONT, font); LineBreakMeasurer
			 * lineBreaker = new LineBreakMeasurer(text.getIterator(), frc);
			 * //每行字符显示长度(点) double width = pf.getImageableWidth();
			 * //首字符的基线位于用户空间中的 (x, y) 位置处 Point2D.Double pen = new
			 * Point2D.Double (100, y+3*heigth); while ( (layout =
			 * lineBreaker.nextLayout( (float) width)) != null){ layout.draw(g2,
			 * (float)x, (float) pen.y); pen.y += layout.getAscent(); }
			 */
			return PAGE_EXISTS;
		default:
			return NO_SUCH_PAGE;
