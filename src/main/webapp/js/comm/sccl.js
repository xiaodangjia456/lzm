...
function addIframe(cur, type) {
	...
	if (!isHas) {
		...
		var iframe = "<iframe class='body-iframe' id='iframe"+m+"' name='iframe" + m
	...
	for ( var i = 0; i < menu.length; i++) {
		...
		try {
			if (item.isHeader == "1") {
				...
				if (item.childMenus == "") {
					str = "<li><a id='"+item.id+"' href='" + item.url
							+ "'><i class='icon-font'>" + item.icon
							+ "</i><span>" + item.name + "</span></a></li>";
					$(parent).append(str);
				} else {
					str = "<li><a id='"+item.id+"' href='"
							+ item.url
							+ "'><i class='icon-font '>"
							+ item.icon
							+ "</i><span>"
